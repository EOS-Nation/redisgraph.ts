...
describe('RedisGraphAPI Test', () =>{
	...
	it('test Query', () => {
		...
		.then( (resultSet) => {
			...
			assert.equal( "roi", record.get(1));
			assert.equal( "roi", record.get("a.name"));
			assert.equal( "32.000000", record.get(0));

			assert.deepStrictEqual( [ 'a.age', 'a.name' ], record.keys());
			assert.deepStrictEqual( [ '32.000000', 'roi' ], record.values());
			assert.equal( false, record.includesKey("aa"));
			assert.equal( true, record.includesKey("a.name"));
